    ...
    <section className="mb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="flex gap-2 mt-4">
            {thumbnails.map((thumb, index) => (
              <img key={index} src={thumb} onClick={() => setCurrentImage(index)}
                className={`w-20 h-16 object-cover rounded-lg cursor-pointer ${currentImage === index ? 'ring-2 ring-emerald-500' : ''}`} />
            ))}
          </div>
        ...
        <Card className="p-6 sticky top-6">
          <div className="text-2xl font-bold">From ₹999 <span className="text-sm font-normal text-gray-600">per person</span></div>
          ...
          <p className="mt-4 text-emerald-600 text-sm">✓ Free cancellation up to 24h before experience starts</p>
        </Card>
      </div>
    </section>
  );
